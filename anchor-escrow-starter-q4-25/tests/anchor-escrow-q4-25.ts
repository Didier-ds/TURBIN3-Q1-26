...
describe("anchor_escrow_q4_25", () => {
  ...
  it("Makes and takes the escrow", async () => {
    ...
    // Setup for take - create ATAs for taker (mintA) and maker (mintB)
    ...
    const setupTx = new anchor.web3.Transaction()
      .add(
        createAssociatedTokenAccountInstruction(provider.wallet.publicKey, takerAtaA, taker.publicKey, mintA),
        createAssociatedTokenAccountInstruction(provider.wallet.publicKey, makerAtaB, maker, mintB)
      );
